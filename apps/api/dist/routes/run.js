...
async function runRoutes(fastify) {
    ...
    fastify.get('/api/run/:id', async (request, reply) => {
        const { id } = request.params;
        const execution = (0, executor_1.getExecution)(id);
        if (!execution) {
        ...
        const pollInterval = setInterval(() => {
            const currentExecution = (0, executor_1.getExecution)(id);
            if (!currentExecution) {
    ...
    fastify.get('/api/run/:id/status', async (request, reply) => {
        const { id } = request.params;
        const execution = (0, executor_1.getExecution)(id);
        if (!execution) {
            return reply.status(404).send({
