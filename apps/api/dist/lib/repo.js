...
async function fetchRepoContext(repoUrl) {
    ...
    // Fetch manifests
    const manifests = {};
    await Promise.all(MANIFEST_FILES.map(async (file) => {
        try {
    ...
    const configFiles = {};
    await Promise.all(CONFIG_FILES.map(async (file) => {
        try {
            const response = await fetch(`${rawBaseUrl}${file}`);
...
function detectLanguages(manifests) {
    const languages = [];
    ...
    if (manifests['composer.json'])
        languages.push('PHP');
    if (manifests['mix.exs'])
        languages.push('Elixir');
    return languages;
}
function analyzeRepository(manifests, readme) {
