...
export interface SecurityScanResult {
  baseScore: number; // 0-100, higher = more dangerous
  categories: Array<{
    name: string;
    pct: number;
    hits: number;
...
interface SecurityRule {
  ...
  pattern: RegExp;
  severity: 'low' | 'medium' | 'high' | 'critical';
  weight: number; // contribution to base score
}
 ...
 */
export function scanRepository(files: Record<string, string>): SecurityScanResult {
  const highlights: SecurityScanResult['highlights'] = [];
