...
export interface SecurityAnalysisResult {
  ...
  explanation: string;
  saferRewrite?: string;
  readiness_score: number; // 0-100, how ready is the repo to run
...
export async function analyzeRepositorySecurity(params: {
  repoUrl: string;
  readme: string;
...
Top Threats:
${params.securityScan.highlights.slice(0, 10).map(h => 
  `  - [${h.severity.toUpperCase()}] ${h.threat} in ${h.path}:${h.line}`
).join('\n')}
...
`;
  ...
  try {
    const result = JSON.parse(responseText) as SecurityAnalysisResult;
