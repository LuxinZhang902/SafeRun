...
import { validatePlan, type Plan } from './planSchema';
...
export async function generatePlan(repoContext: {
  url: string;
  readme: string;
...
export async function summarizeExecution(logs: string[]): Promise<string> {
  const message = await anthropic.messages.create({
    model: 'claude-3-5-sonnet-20241022',
    max_tokens: 1024,
    messages: [
